...
theta0 = 160*u.deg  # rotate clockwise
r0 = 1750*u.au
phi0 = 120*u.deg  # rotate the plane
v_r0 = 0*u.km/u.s
omega0 = 8e-13/u.s

# Arrays
thetalist = np.array([140, 145, 150, 155, 160]) * u.deg
rlist = np.array([1250, 1500, 1750, 2000])* u.au
philist = np.array([100, 110, 120, 130])* u.deg
# rlist =np.array([1600, 1800, 2000]) * u.au
v_rlist = np.array([0, 0.1, 0.2, 0.3]) * u.km/u.s
omegalist = np.array([3., 4.,5.,6., 7.])* 1.e-13 / u.s
...
# For a double comparison, the lists must be 2-3 members each, no more
# Arrays for double comparison
thetalist = np.array([160, 170]) * u.deg
# rlist = np.array([1250, 1500, 1750, 2000])* u.au
philist = np.array([100, 110, 120])* u.deg
# rlist =np.array([1600, 1800, 2000]) * u.au
# v_rlist = np.array([0, 0.1, 0.2, 0.3]) * u.km/u.s
# omegalist = np.array([3., 4.,5.,6., 7.])* 1.e-13 / u.s
for phi0 in philist:
