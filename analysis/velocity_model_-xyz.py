...
in this code, i=0 is an edge on disk and a PA=0 is in east to west direction
...
savekernel = False  # if True, saves the KDE for the velocities
savemodel = False  # if True, saves the resulting streamline model (only use for single parameters)
...
# inc = -(43) * u.deg
# PA_ang = (20+90)*u.deg
regionsample = 'data/region_streamer_l_kink.reg'  # 'data/region_streamer_C18O_final.reg'
imagename = H2CO_303_202_fit_Tpeak  # C18O_2_1_TdV
vcname = H2CO_303_202_fit_Vc  # C18O_2_1_fit_Vc
...
# Fixed parameter
v_lsr = 7.48*u.km/u.s  # +- 0.14 km/s according to our C18O data

Per50_c = SkyCoord(ra_Per50, dec_Per50, frame='fk5')
Per50_ref = Per50_c.skyoffset_frame()

# figure 1: 3d plot to see the 3d structure of the streamline (if wanted)
fig = plt.figure()
...
# First we add the axes to the image plane to see the axes we are working with
...
if ny_b[-1] > 0:
    new_ax_color = 'red'  # away from the observer
else:
    new_ax_color = 'blue'  # towards the observer
...
# We obtain the kernel density estimation for r and v

# Obtain the offset radius from Per-emb-50 and the v_lsr for each
# pixel in the streamer region
r_proj, v_los = per_emb_50_get_vc_r('../'+vcname+'.fits',
                                    '../'+regionsample)
...
ax3.contourf(xx, yy, zz, cmap='Greys', levels=np.arange(0.1, 1.2, 0.1),
             vmin=0., vmax=1.1)
...
'''
If you want to run the streamline model with one set of parameters, use this
block
'''
...
'''
# If you want to test one array of parameters, use this block
...
# for mt in M_totlist:
    ...
    my_label = stream_label(omega=omega0, theta=theta0, phi=phi0+5*u.deg,
                            r=r0, v_r=v_r0)
    # we obtain the distance of each point in the sky
    d_sky_au = np.sqrt(x1**2 + z1**2)
    # Stream line into arcsec
...
'''
If you want to test more than one array at the same time, use this block


thetalist = np.array([73,75,78,81,84]) * u.deg
philist = np.array([0,5,10,15,20]) * u.deg
colorlist = np.array(['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728',
